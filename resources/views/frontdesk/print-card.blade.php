...
@php
    ...
    $textY = static function (array $element, array $lines): int {
        ...
        $lineHeight = (int) round($fontSize * 1.1);
...
<body>
    <div class="preview-stack">
        <div class="preview-meta">
            ...
            <span>PNG render · 300 dpi · 1011 × 638 px</span>
        </div>

        <div class="preview-status" id="preview-status">PNG render voorbereiden…</div>

        <div class="print-sheet">
            <img id="print-image" class="print-image" data-ready="0" alt="Kaartafdruk" />
            <canvas id="print-canvas" class="print-canvas" width="{{ $svgWidth }}" height="{{ $svgHeight }}"></canvas>
        </div>
    </div>
    ...
    <script>
        const svgNode = document.getElementById('svg-source')
        ...
        function updateStatus(message, isError = false) {
            ...
            statusNode.style.borderColor = isError ? 'rgba(239, 68, 68, 0.35)' : 'rgba(148, 163, 184, 0.25)'
        ...
        function buildPngFromSvg() {
            return new Promise((resolve, reject) => {
                try {
                    ...
                    rasterImage.onerror = (error) => {
                        URL.revokeObjectURL(blobUrl)
                        reject(error)
                    }

                    rasterImage.src = blobUrl
                } catch (error) {
                    reject(error)
                }
            })
        ...
        async function preparePrint() {
            try {
                updateStatus('PNG render voorbereiden…')
                const pngUrl = await buildPngFromSvg()
                ...
                updateStatus('PNG render klaar · printen…')
                setTimeout(() => window.print(), 160)
            } catch (error) {
                console.error(error)
                updateStatus('PNG render mislukt. Open de kaart opnieuw en probeer nog eens.', true)
                image.dataset.ready = '1'
            }
        }
