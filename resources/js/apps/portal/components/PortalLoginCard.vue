<template>
    <div class="flex min-h-screen items-center justify-center bg-slate-950 p-6">
        <div class="w-full max-w-md rounded-3xl border border-slate-800 bg-slate-900 p-8 shadow-2xl">
            ...
            <h1 class="mt-3 text-3xl font-bold text-white">Portal</h1>
            <p class="mt-2 text-slate-400">Beheer je publieke venuepagina.</p>
...
async function submit() {
    ...
    try {
        ...
        error.value = err?.data?.errors?.username?.[0]
            || err?.data?.message
