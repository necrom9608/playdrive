        ...
        <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative z-10 flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-3xl border border-slate-800 bg-slate-900 shadow-2xl">
                ...
                <div class="flex-1 overflow-y-auto p-6">
                    ...
                    <!-- Standaard bekijken tab -->
                    <div v-else-if="activeTab === 'default'" class="space-y-4">
                        <div class="rounded-2xl border border-slate-700 bg-slate-950/60 p-4 text-sm text-slate-400">
                            Dit is de standaardtekst zoals ingesteld door PlayDrive (of de beheerder). Je eigen aanpassingen overschrijven deze tekst.
                        </div>
                        <div>
                            <label class="mb-1 block text-sm font-medium text-slate-400">Standaard onderwerp</label>
                            <div class="rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-sm text-slate-300 font-mono">
                                {{ template?.default_subject ?? '—' }}
                            </div>
                        </div>
                        <div>
                            <label class="mb-1 block text-sm font-medium text-slate-400">Standaard inhoud</label>
                            <textarea
                                :value="template?.default_body ?? ''"
                                readonly
                                rows="12"
                                class="w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 font-mono text-sm text-slate-400 outline-none cursor-default resize-none"
                            />
                        </div>
                        <button
                            type="button"
                            class="rounded-xl border border-slate-700 px-4 py-2.5 text-sm font-medium text-slate-300 transition hover:bg-slate-800"
                            @click="copyDefaultToForm"
                        >
                            Standaard als startpunt gebruiken →
                        </button>
                    </div>
                </div>
...
const tabs = [
    ...
    { key: 'default', label: 'Standaard bekijken' },
]
...
watch(activeTab, (tab) => {
    if (tab === 'preview' && !previewHtml.value) loadPreview()
})
...
function formatVariable(variable) { return '{{' + variable + '}}' }
...
function copyDefaultToForm() {
    form.subject    = props.template?.default_subject ?? ''
    form.body       = props.template?.default_body    ?? ''
    activeTab.value = 'edit'
}
