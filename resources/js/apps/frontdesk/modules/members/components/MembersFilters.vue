    ...
    <div class="rounded-3xl border border-slate-800 bg-slate-900 p-4 shadow-xl">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="grid flex-1 gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
                <label class="space-y-2 text-sm text-slate-300">
                    ...
                    <input
                        :value="search"
                        type="text"
                        placeholder="Naam, e-mail of RFID"
                        ...
                        >
                            <div class="space-y-3">
                                ...
                                >
                                    ...
                                    >
                                    <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold" :class="option.class">{{ option.label }}</span>
                                </label>
                            </div>
                            <div class="mt-4 flex justify-between gap-3 border-t border-slate-800 pt-4">
                                <button type="button" class="rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm font-semibold text-slate-200 transition hover:bg-slate-800" @click="$emit('update:selected-statuses', []); isOpen = false">Wissen</button>
                                <button type="button" class="rounded-xl bg-blue-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-blue-500" @click="isOpen = false">Klaar</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-wrap gap-3">
                <button type="button" class="rounded-2xl border border-slate-700 bg-slate-800 px-4 py-3 text-sm font-semibold text-slate-200 transition hover:bg-slate-700" @click="$emit('search')">Zoeken</button>
                <button type="button" class="rounded-2xl border border-cyan-500/30 bg-cyan-500/10 px-4 py-3 text-sm font-semibold text-cyan-200 transition hover:bg-cyan-500/20" @click="$emit('new-display')">Nieuw lid via display</button>
            </div>
        </div>
    </div>
</template>
...
const emit = defineEmits(['update:search', 'update:selected-statuses', 'search', 'new-display'])

const isOpen = ref(false)

const statusOptions = [
    { value: 'none',     label: 'Geen abonnement',    class: 'bg-cyan-500/15 text-cyan-300' },
...
const selectedBadges = computed(() => statusOptions.filter(o => props.selectedStatuses.includes(o.value)))

function toggleStatus(value) {
    const next = props.selectedStatuses.includes(value)
        ? props.selectedStatuses.filter(s => s !== value)
