...
class PhysicalCardRenderData
{
    ...
    public static function loadRelations(): array
    {
        return [
            ...
            'staffHolder:id,name,username,email,rfid_uid,is_admin',
            // memberHolder is nu TenantMembership; account wordt via $with meegeladen
            'memberHolder',
            'memberHolder.account:id,first_name,last_name,email',
        ];
    }
    ...
    private static function resolveTemplateAndFields(PhysicalCard $card): array
    {
        ...
        if ($type === PhysicalCard::TYPE_MEMBER) {
            // memberHolder is nu een TenantMembership met accessors voor first_name/last_name/email
            $membership = $card->memberHolder;
            $firstName  = $membership?->first_name;
            $lastName   = $membership?->last_name;
            $fullName   = trim(($firstName ?? '') . ' ' . ($lastName ?? ''));

            $validUntil = $membership?->membership_ends_at
                ? $membership->membership_ends_at->format('d/m/Y')
                : '—';

            $fields = [
                'full_name'       => $fullName ?: ($card->label ?: 'Member badge'),
                'first_name'      => $firstName ?: self::firstName($fullName),
                'last_name'       => $lastName ?: self::lastName($fullName),
                'membership_type' => ucfirst($membership?->membership_type ?? 'Member'),
