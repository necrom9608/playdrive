...
class MailLogController extends Controller
{
    public function index(Request $request, CurrentTenant $currentTenant): JsonResponse
    {
        ...
        $summary = [
            'total'      => ($base)()->count(),
            'sent_today' => ($base)()->whereDate('created_at', today())->count(),
        ...
        return response()->json([
            'data' => [
                'summary' => $summary,
                'logs'    => collect($logs->items())->map(fn (MailLog $log) => $this->map($log))->values(),
                'meta'    => [
    ...
    public function show(CurrentTenant $currentTenant, MailLog $mailLog): JsonResponse
    {
        ...
        $mailLog->loadMissing('account:id,first_name,last_name,email');
    ...
    private function map(MailLog $log, bool $withBody = false): array
    {
        return [
            ...
            'account_name'       => $log->account
                ? trim(($log->account->first_name ?? '') . ' ' . ($log->account->last_name ?? ''))
                : null,
            'subject'            => $log->subject,
