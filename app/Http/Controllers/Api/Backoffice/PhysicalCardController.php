...
class PhysicalCardController extends Controller
{
        ...
        if ($search !== '') {
            $query->where(function (Builder $builder) use ($search) {
                $builder
                    ...
                    ->orWhereHas('voucherTemplate', fn (Builder $q) => $q->where('name', 'like', '%' . $search . '%'))
                    ->orWhereHas('badgeTemplate', fn (Builder $q) => $q->where('name', 'like', '%' . $search . '%'))
                    ->orWhereHas('staffHolder', function (Builder $q) use ($search) {
                        $q->where('name', 'like', '%' . $search . '%')
                            ->orWhere('username', 'like', '%' . $search . '%')
                            ->orWhere('email', 'like', '%' . $search . '%');
                    })
                    // Zoek op naam/email via memberHolder (TenantMembership) → account
                    ->orWhereHas('memberHolder.account', function (Builder $q) use ($search) {
        ...
        return response()->json([
            'data' => [
                ...
                'voucher_templates' => VoucherTemplate::query()
                    ...
                    ->orderBy('name')
                    ->get()
                    ->map(fn (VoucherTemplate $t) => $this->transformVoucherTemplate($t))
                    ->values(),
                'badge_templates' => BadgeTemplate::query()
                    ...
                    ->orderBy('name')
                    ->get()
                    ->map(fn (BadgeTemplate $t) => $this->transformBadgeTemplate($t))
                    ->values(),
                'staff' => User::query()
                    ->where('tenant_id', $currentTenant->id())
                    ->orderBy('name')
                    ->get(['id', 'name', 'username', 'email', 'rfid_uid'])
                    ->map(fn (User $u) => $this->transformStaffOption($u))
                    ->values(),
                // Leden: via TenantMembership → Account
                'members' => TenantMembership::query()
                    ->where('tenant_id', $currentTenant->id())
                    ->with('account:id,first_name,last_name,email')
                    ->orderBy('id')
                    ->get()
                    ->sortBy(fn ($m) => strtolower(($m->account?->last_name ?? '') . ' ' . ($m->account?->first_name ?? '')))
                    ->map(fn (TenantMembership $m) => $this->transformMembershipOption($m))
                    ->values(),
    ...
    public function destroy(Request $request, CurrentTenant $currentTenant, PhysicalCard $card): JsonResponse
    {
        ...
        return response()->json(['message' => 'Kaart verwijderd.']);
    }
    ...
    public function uploadRenderImage(Request $request, CurrentTenant $currentTenant, PhysicalCard $card): JsonResponse
    {
        ...
        $data   = $request->validate(['data_url' => ['required', 'string']]);
    ...
    public function markPrinted(Request $request, CurrentTenant $currentTenant, PhysicalCard $card): JsonResponse
    {
        abort_unless((int) $card->tenant_id === (int) $currentTenant->id(), 404);
        abort_if(
            blank($card->render_image_path) || ! Storage::disk('public')->exists($card->render_image_path),
            422,
            'Voor deze kaart is nog geen PNG-preview beschikbaar. Sla de kaart opnieuw op of laad de preview opnieuw.'
        );
    ...
    public function pdf(Request $request, CurrentTenant $currentTenant, PhysicalCard $card, SimpleImagePdfService $pdfService)
    {
        ...
        abort_if(
            blank($card->render_image_path) || ! Storage::disk('public')->exists($card->render_image_path),
            404,
            'Voor deze kaart is nog geen render-afbeelding beschikbaar.'
        );
    ...
    protected function validatePayload(Request $request, CurrentTenant $currentTenant, ?int $cardId): array
    {
        ...
        if (($data['card_type'] ?? null) === PhysicalCard::TYPE_MEMBER) {
            ...
            // holder_id verwijst nu naar tenant_memberships.id
            abort_unless(
    ...
    protected function transformCard(PhysicalCard $card): array
    {
        ...
        if ($card->card_type === PhysicalCard::TYPE_STAFF) {
            ...
            // memberHolder is TenantMembership, naam via accessors
            $holderName = $card->memberHolder?->full_name ?: null;
        }
        ...
        return [
            ...
            'badge_template_name'        => $card->card_type === PhysicalCard::TYPE_VOUCHER
                ? $card->voucherTemplate?->badgeTemplate?->name
                : $card->badgeTemplate?->name,
            ...
            'preview_image_url'          => $card->render_image_path
                ? Storage::disk('public')->url($card->render_image_path) . '?v=' . urlencode((string) optional($card->updated_at)->timestamp)
                : null,
            'render_image_path'          => $card->render_image_path,
            'render_image_download_name' => 'card-' . $card->id . '.png',
    ...
    protected function resolvedRfidUid(array $data, CurrentTenant $currentTenant): string
    {
        ...
        if (($data['card_type'] ?? null) === PhysicalCard::TYPE_MEMBER && $holderId > 0) {
            // rfid_uid zit nu op TenantMembership
            $rfidUid = trim((string) TenantMembership::query()
                ->where('tenant_id', $currentTenant->id())
                ->where('id', $holderId)
                ->value('rfid_uid'));
        }
    ...
    protected function transformMembershipOption(TenantMembership $membership): array
    {
        return [
            'id'         => $membership->id,
